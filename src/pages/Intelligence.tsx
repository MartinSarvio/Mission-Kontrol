...
const sourceConfig = {
  x: { label: 'X', bg: 'bg-black', text: 'text-white' },
...
const mockResearch: ResearchItem[] = [
  {
    ...
    citations: [{ title: 'Restaurant Tech Trends 2026', url: '#', source: 'LinkedIn' }, { title: 'AI Ordering Adoption Rates', url: '#', source: 'X' }],
  },
  {
  ...
  },
  {
    ...
    citations: [{ title: 'Restaurant SMS Marketing Benchmarks', url: '#', source: 'X' }],
  },
  {
  ...
  },
  {
    ...
    citations: [{ title: 'Dynamic Pricing for Restaurants', url: '#', source: 'X' }],
  },
  {
    ...
    citations: [{ title: 'PWA vs Native App for Restaurants', url: '#', source: 'X' }],
  },
  {
  ...
  },
  {
    ...
    citations: [{ title: 'Supabase vs Firebase for SaaS 2026', url: '#', source: 'X' }],
  },
]
...
export default function Intelligence() {
  ...
  function openItem(item: ResearchItem) { toggleRead(item.id); setSelectedItem(item) }
  ...
  return (
    <div className="relative">
      ...
      {showConfig && (
        <div className="card mb-6 mt-4" style={{ border: '1px solid rgba(0,122,255,0.15)' }}>
      ...
      <div className="mt-5 space-y-1">
        {items.map(item => (
          <div key={item.id} onClick={() => openItem(item)}
            ...
            style={!item.read ? { backdropFilter: 'blur(16px)', background: 'rgba(0,122,255,0.04)', border: '1px solid rgba(0,122,255,0.08)' } : {}}>
            <button onClick={e => toggleStar(item.id, e)}
              ...
              <Icon name={item.starred ? 'star-fill' : 'star'} size={18} />
            </button>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                <h3 className={`text-sm truncate ${item.read ? 'font-medium' : 'font-bold'}`} style={{ color: item.read ? '#636366' : '#1d1d1f' }}>{item.subject}</h3>
                {!item.read && <span className="w-2 h-2 rounded-full bg-blue-500 flex-shrink-0" />}
              </div>
              <p className="text-xs line-clamp-1" style={{ color: '#86868b' }}>{item.preview}</p>
            </div>
            ...
            <span className="text-[11px] flex-shrink-0 w-16 text-right" style={{ color: '#86868b' }}>
      ...
      {selectedItem && (
        <>
          <div className="fixed inset-0 glass-overlay z-50" onClick={() => setSelectedItem(null)} />
          <div className="fixed right-0 top-0 h-full w-[520px] glass-panel z-50 overflow-y-auto animate-slide-in">
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <button onClick={() => setSelectedItem(null)} className="w-8 h-8 flex items-center justify-center rounded-full" style={{ background: 'rgba(0,0,0,0.04)', color: '#86868b' }}>
