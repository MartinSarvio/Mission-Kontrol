  ...
  if (!q) {
    ...
    if (category === 'all' || category === 'clients') {
      clients.filter(c => c.name.toLowerCase().includes(q)).forEach(c =>
        results.push({ type: 'Client', title: c.name, subtitle: c.company, status: c.status }))
    }
    if (category === 'all' || category === 'documents') {
      documents.filter(d => d.name.toLowerCase().includes(q)).forEach(d =>
        results.push({ type: 'Document', title: d.name, subtitle: `${d.type} · v${d.version}` }))
    }
    ...
    if (category === 'all' || category === 'cron') {
      cronJobs.filter(c => c.name.toLowerCase().includes(q)).forEach(c =>
        results.push({ type: 'Cron Job', title: c.name, subtitle: c.schedule, status: c.status }))
    }
  }

  return (
    <div>
      <h1 className="page-title mb-1">Index</h1>
      <p className="caption mb-6">Universal search across all entities</p>

      <div className="max-w-2xl mx-auto">
        <SearchBar value={search} onChange={setSearch} placeholder="Search everything..." />
        ...
        {!search && (
          <div className="text-center py-16">
            ...
            <p className="text-apple-gray-400">Start typing to search across agents, clients, documents, journal entries, and cron jobs</p>
          </div>
        )}

        {search && results.length === 0 && (
          <div className="text-center py-16">
            <p className="text-apple-gray-400">No results found for "{search}"</p>
          </div>
        )}
