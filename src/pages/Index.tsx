        ...
        <div className="flex flex-wrap gap-1 mt-4 mb-6">
          {(['all', 'agents', 'clients', 'documents', 'journal', 'cron'] as Category[]).map(c => (
            <button key={c} onClick={() => setCategory(c)}
              className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-all whitespace-nowrap ${category === c ? 'bg-apple-blue text-white' : 'px-4 py-2 rounded-xl text-sm font-medium bg-white/5 border border-white/10 text-white'}`}
            ...
            <Card key={i} className="cursor-pointer">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <span className="px-2 py-0.5 rounded-lg text-[11px] font-medium whitespace-nowrap" style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.4)' }}>{r.type}</span>
                  <div>
